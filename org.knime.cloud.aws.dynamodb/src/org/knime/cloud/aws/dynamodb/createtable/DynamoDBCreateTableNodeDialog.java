...
final class DynamoDBCreateTableNodeDialog extends NodeDialogPane {
    ...
    private JCheckBox m_hasRangeKey = new JCheckBox("Range Key");
    private JTextField m_rangeKeyName = new JTextField(10);
    private EnumComboBox<ScalarAttributeType> m_rangeKeyType
        = new EnumComboBox<>(ScalarAttributeType.values(), DynamoDBUtil.getHumanReadableKeyTypes());
    
    private EnumComboBox<BillingMode> m_billingMode = new EnumComboBox<>(BillingMode.values(),
            new String[] {BILLING_MODE_PROVISIONED, BILLING_MODE_PAY_PER_REQ});

    private JSpinner m_readUnits = new JSpinner(new SpinnerNumberModel(5, 1, MAX_CAPACITY_UNITS, 1));
    private JSpinner m_writeUnits = new JSpinner(new SpinnerNumberModel(5, 1, MAX_CAPACITY_UNITS, 1));
    
    private JCheckBox m_blockUntilActive = new JCheckBox("Block until table is active");
    
    private DefaultTableModel m_tagsModel = new DefaultTableModel(new String[] {"Name", "Value"}, 1);

    private JPanel m_indexesTab = new JPanel(new BorderLayout());
    private JPanel m_indexes = new JPanel();
    
    // SSE
    private JCheckBox m_sseEnabled = new JCheckBox("Enable Server-Site Encryption");
    private JTextField m_kmsMasterKeyId = new JTextField();
    
    // Streams
    private JCheckBox m_streamsEnabled = new JCheckBox("Enable Streams");
    private EnumComboBox<StreamViewType> m_streamType = new EnumComboBox<>(StreamViewType.values(), new String[] {
    ...
    private JPanel createSSEPanel() {
        ...
        c.gridy++;
        c.gridwidth = 2;
        JLabel lbl = new JLabel("(only specify if different from the default DynamoDB customer master key)");
    ...
    private JPanel createTagsTab() {
        ...
        JTable tagsTbl = new JTable(m_tagsModel);
        ...
        JScrollPane tagsScrollPane = new JScrollPane(tagsTbl);
        panel.add(tagsScrollPane);
        
        JButton deleteTag = new JButton("-");
        deleteTag.addActionListener(e -> {
            int mx = tagsTbl.getSelectionModel().getMaxSelectionIndex();
            int mn = tagsTbl.getSelectionModel().getMinSelectionIndex();
            for (int i = mx; i >= mn; i--) {
    ...
    private JPanel createIndexesTab() {
        ...
        JButton addGlobalIdx = new JButton("Add Global Index");
        JButton addLocalIdx = new JButton("Add Local Index");
        ...
        JScrollPane scrollPane = new JScrollPane(m_indexes);
    ...
    private JPanel tableSettingsPanel() {
        ...
        m_billingMode.addActionListener(e -> {
            BillingMode mode = m_billingMode.getSelectedItemValue();
            ...
            for (Component comp : m_indexes.getComponents()) {
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) throws InvalidSettingsException {
        ...
        for (int i = 0; i < m_tagsModel.getRowCount(); i++) {
            String name = (String)m_tagsModel.getValueAt(i, 0);
            String val = (String)m_tagsModel.getValueAt(i, 1);
        ...
        for (Component c : m_indexes.getComponents()) {
            if (c instanceof IndexPanel) {
                IndexSettings idx = ((IndexPanel)c).createIndexSettings();
